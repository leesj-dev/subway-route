...
증명 순서는 다음과 같습니다.
1. `crawling.py`: 위키백과의 '[수도권 전철역 목록](https://ko.wikipedia.org/wiki/%EC%88%98%EB%8F%84%EA%B6%8C_%EC%A0%84%EC%B2%A0%EC%97%AD_%EB%AA%A9%EB%A1%9D)'을 크롤링 및 파싱하여 `station_line.py`로 저장 (*도라산역은 제외함)
2. `station_line.py`에서 운행 중단된 도라산역 제외
3. `main.py`: BFS(너비 우선 탐색)를 수행하여 결과값을 `result.txt`로 저장
...
$f : X = \\{ x \\: | \\: x\textrm{ 는 수도권 지하철역}\\} \rightarrow  Y = \\{ y \\: | \\: y\textrm{ 는 역을 통과하는 노선들의 집합}\\}$
$g : X = \\{ x \\: | \\: x\textrm{ 는 수도권 지하철 노선}\\}  \rightarrow  Y = \\{ y \\: | \\: y\textrm{ 는 그 노선을 통과하는 역들의 집합}\\}$

여기서 헷갈리지 말아야 할 점은 $f$와 $g$의 <b>공역의 원소 각각이 하나의 '집합'</b>이라는 것입니다. 예컨대 $f(\textrm{교대}) = \\{2,\\:3\\}$ 이고, $g(8) = \\{\textrm{암사},\\:\textrm{천호},\\:\cdots,\\:\textrm{모란}\\}$ 입니다.

$h : X = \\{ x \\: | \\: x\textrm{ 는 수도권 지하철역}\right\\} \rightarrow  Y = \\{ y \\: | \\: y\textrm{ 는 출발역으로부터 그 역까지 최소환승으로 갔을 때 거친 노선 수}\\}$
...
(ii) $m$을 지나는 역을 $g(m)$을 통해 구합니다. $g(m)$의 원소 $s_{1},\\:s_{2},\\:\cdots,\\:s_{i}$ 에 대하여 $s_{i}$ 를 이미 지나가지 않았다면 $h(x_i) = h(S_k) + 1$ 로 구하고, 이미 지나갔다면 마지막 단계로 넘어갑니다.

(iv) $s_{i}$ 를 지나는 노선을 $f(s_i)$를 통해 구합니다. $f(s_i)$의 원소 $s_{i,\\,1},\\:s_{i,\\,2},\\:\cdots,\\:s_{i,\\,j}$ 에 대하여, $s_{i,\\,j}$ 가 $V$의 원소가 아니라면 $Q$에 $s_{i,\\,j}$ 을 추가합니다.
...
전체 역 수는 $650$개이므로, 검산해보면 이론상 가능한 조합의 수 ${}_{650}\mathrm{C}_2 = 210925$와 동일함을 알 수 있습니다.

<br>

## 업데이트 내역
* 2024.08.18. 8호선 별내↔암사 구간 개통 (2024.08.10.) 반영
* 2024.04.14. GTX-A 수서↔동탄 구간 개통 (2024.04.01.) 반영
